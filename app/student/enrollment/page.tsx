...
import CustomEnrollmentScheduler from "@/app/parent/children/[id]/enrollment/CustomEnrollmentScheduler";
...
export default async function StudentEnrollmentPage({
  ...
  if (!user) {
    redirect(
      "/login?next=%2Fstudent%2Fenrollment"
    );
  }
  ...
  /*
   * levelTestId가 전달된 경우에는
   * 현재 로그인한 수강생 본인의
   * 직접 레벨테스트만 추천 과정에 반영합니다.
   *
   * child_id가 있는 학부모 관리형 레벨테스트는
   * 기존 학부모 수강신청 경로에서 처리합니다.
   */
  ...
  const backHref =
    validLevelTestId
      ? `/parent/level-tests/${validLevelTestId}?studentMode=1`
      : "/student";
