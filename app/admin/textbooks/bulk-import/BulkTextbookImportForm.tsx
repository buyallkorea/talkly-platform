...
async function readWorkbook(file: File): Promise<ImportRow[]> {
  ...
  /*
   * 중요:
   * 첫 번째 행을 헤더라고 가정하지 않고
   * 시트 전체를 2차원 배열로 읽습니다.
   *
   * 이렇게 하면 엑셀 상단에 제목/안내 행이 있어도
   * 실제 '교재명' 헤더 행을 자동으로 찾을 수 있습니다.
   */
  ...
  if (headerIndex < 0) {
    throw new Error(
      "'교재명' 헤더를 찾을 수 없습니다. 엑셀의 '교재등록' 시트를 확인해주세요."
    );
  }

  const header = matrix[headerIndex].map((cell) =>
    textValue(cell)
  );
    ...
    >
      ...
      {result && (
        ...
        >
          ...
          >
            전체 {result.total}종 · 갱신{" "}
            {result.updated}종 · 미매칭{" "}
            {result.unmatched}종 · 실패{" "}
            {result.failed}종
          </p>
