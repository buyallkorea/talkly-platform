...
export default async function AdminChildStudentDetailPage({
  params,
}: PageProps) {
  ...
  const holdMap = new Map<number, (typeof holds)[number]>();

  for (const hold of holds) {
    if (!holdMap.has(hold.class_session_id)) {
      holdMap.set(hold.class_session_id, hold);
    }
  }
  ...
  const pendingHoldCount = holds.filter(
