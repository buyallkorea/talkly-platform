  ...
  const content = (
    <div
      style={{
        ...
        background: "#ffffff",
        boxShadow:
          "0 1px 2px rgba(16,24,40,0.03), 0 8px 24px rgba(16,24,40,0.05)",
        minHeight: "132px",
...
export default async function AdminDashboardPage() {
  ...
  tomorrowStartDate.setDate(
    tomorrowStartDate.getDate() + 1
  );

  const day =
    todayStartDate.getDay();
  ...
  monthEndDate.setMonth(
    monthEndDate.getMonth() + 1
  );

  const todayStart =
    todayStartDate.toISOString();

  const tomorrowStart =
    tomorrowStartDate.toISOString();
  ...
  const todaySessionRows =
    (todaySessionRowsResult.data ??
      []) as TodaySessionRow[];

  const todaySessionIds =
    todaySessionRows.map(
      (session) => session.id
    );
  ...
  const todayAttendanceBase =
    todayPresentCount +
    todayAbsentCount;

  const todayAttendanceRate =
    todayAttendanceBase > 0
      ? Math.round(
          (todayPresentCount /
            todayAttendanceBase) *
            100
  ...
  const alerts = [
...

    {
      ...
      value:
        unassignedEnrollmentsResult.count ??
        0,
