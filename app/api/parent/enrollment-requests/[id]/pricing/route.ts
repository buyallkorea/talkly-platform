...
export async function POST(
  ...
  const enrollmentRequestId = Number(id);

  if (
    !Number.isInteger(enrollmentRequestId) ||
    enrollmentRequestId <= 0
  ) {
    return NextResponse.json(
      {
        error: "수강신청 번호가 올바르지 않습니다.",
      },
      {
        status: 400,
      }
  ...
  if (enrollmentRequestError) {
    return NextResponse.json(
      {
        error: enrollmentRequestError.message,
      },
      {
        status: 500,
      }
  ...
  const [teacherResult, policyResult] =
    await Promise.all([
      adminClient
  ...
  const monthlyLessonCount = Number(
    pricing.monthly_lesson_count ?? 0
  );
  ...
  const weekdayDayCount = Math.max(
    0,
    lessonsPerWeek - weekendDayCount
  );

  const weeksPerMonth =
    monthlyLessonCount / lessonsPerWeek;

  const monthlyWeekdayLessons =
    weekdayDayCount * weeksPerMonth;

  const monthlyWeekendLessons =
    weekendDayCount * weeksPerMonth;
  ...
  const discountRate = Number(
    policyResult.data.discount_rate
  );
